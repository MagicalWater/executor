        ...
        server.once("error", (error: unknown) => {
          ...
          const code =
            typeof error === "object" && error !== null && "code" in error
              ? String(error.code)
              : null;
          // If this address family is unavailable on the host, it cannot own
          // the requested listener. Treat that family as released while still
          // failing closed for EADDRINUSE and every other bind failure.
